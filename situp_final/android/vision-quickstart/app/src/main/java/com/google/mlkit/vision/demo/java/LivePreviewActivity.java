...
public final class LivePreviewActivity extends AppCompatActivity
    ...
    private String selectedModel = POSE_DETECTION; // 默认选中
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btn_reset).setOnClickListener(view -> {
            PoseClassifierProcessor.getInstance().setPauseCounting();
        });
        TextView textView = findViewById(R.id.text);
        Handler handler = new Handler(getMainLooper());
        // 点击开始
        findViewById(R.id.btn_start).setOnClickListener(view -> {
            view.setEnabled(false); // 不克重复点击
            final int[] i = {3};
            Runnable r = new Runnable() {
                ...
                public void run() {
                    if (i[0] == 0) { // 倒计时结束
                        textView.setText("Go");
                        view.setEnabled(true); // 开启点击
                        PoseClassifierProcessor.getInstance().resetCounting(); // 开始计数
                        handler.postDelayed(()->{textView.setText("");},1000); // 倒计时半秒删除go
                    } else { // 正在倒计时
